...
REFERENCE_PROSPECTUS_DOCX = Path(
    r"C:\Users\12534\Desktop\基金合同\南方中证全指红利质量交易型开放式指数证券投资基金招募说明书.docx"
)
...
class ProspectusEngine:
    def __init__(self):
        ...
        self._reference_fixed_cache = None
...
3、基金管理人可根据市场情况，在法律法规允许的情况下，合理调整上述申购和赎回的数量或比例限制，并在实施前依照《信息披露办法》的有关规定在规定媒介上公告。"""
    ...
    # ── Step 4-5: 条件处理 + 占位符替换（委托给 ContractEngine）───────────
    def _process_conditionals(self, text: str, v: dict) -> str:
    ...
    def _cleanup(self, text: str) -> str:
        ...
        for line in lines:
            if skip_header:
                if line.strip().startswith("{FUND_NAME}") or (
                    "招募说明书" in line and "模板说明" not in line
    ...
    def generate(self, form_data: dict) -> str:
        v = self._derive_variables(form_data)
        v = self._inject_clause_texts(v)
        v = self._extract_contract_sections(v)   # 提取合同各部分供招募说明书引用
    ...
    def build_docx_prospectus(self, prospectus_text: str) -> bytes:
        ...
        # 正则（章 格式，非 部分 格式）
        RE_TOC_ENTRY = re.compile(r"^(第[一二三四五六七八九十百]+章\s+.+?)[\t ]+(\d+)$")
        RE_CHAP_HEAD = re.compile(r"^第[一二三四五六七八九十百]+章\s+\S")
        RE_UPPER_NUM = re.compile(r"^[一二三四五六七八九十百]+、")
        RE_PAREN_CN  = re.compile(r"^（[一二三四五六七八九十百]+）")
        RE_ARAB_DOT  = re.compile(r"^\d+、")
        RE_PAREN_NUM = re.compile(r"^（\d+）")
        ...
        def _chap_anchor(heading_s: str) -> str:
            m = re.match(r'^第([一二三四五六七八九十百]+)章', heading_s)
            if not m:
                return ""
            chap_no = ContractEngine._cn_numeral_to_int(m.group(1))
            return f"chap_{chap_no}" if chap_no is not None else ""

        lines = prospectus_text.split("\n")
        phase       = "cover"
        cover_idx   = 0
        current_chap = 0
        ...
        for raw in lines:
            ...
            # 章标题
            if phase == "body" and RE_CHAP_HEAD.match(s):
                current_chap += 1
                p = doc.add_paragraph(style="Heading 1")
                _set_para(p, jc="center", line=360, before=0, after=0,
                          keep_lines=True, page_break_before=True)
                anchor = _chap_anchor(s)
                if anchor:
                    _bm_id[0] += 1
                    bm_start = OxmlElement("w:bookmarkStart")
                    bm_start.set(qn("w:id"), str(_bm_id[0]))
...
def api_generate_prospectus():
    ...
    try:
        ...
        return jsonify({"success": True, "text": prospectus_text})
    except Exception as e:
        import traceback
        return jsonify({"success": False, "error": str(e), "trace": traceback.format_exc()}), 400
